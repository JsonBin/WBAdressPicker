...
import UIKit
typealias  selectBlock = (dressArray:NSArray) -> Void
class WBAdressPicker: UIView, UIPickerViewDelegate, UIPickerViewDataSource {
    
    internal var showTown=Bool()  // 设置是否显示区县，默认为false.
    internal var pickArray:NSArray? // 第一次传入已有的地址，跳转到选择好的位置

    private let picker=UIPickerView()
    private var block:selectBlock?
    ...
    class var shareInstance: WBAdressPicker {
        struct Static {
            static var onceToken:dispatch_once_t=0
            static var instance:WBAdressPicker?=nil
        }
        dispatch_once(&Static.onceToken) { 
            Static.instance=WBAdressPicker()
    ...
    internal func selectAdress(adressArray:selectBlock)->Void{
        block=adressArray
    }
    ...
    internal func show(view:UIView)->Void{
        ...
        UIView.animateWithDuration(0.3) { 
            self.bgView.frame=CGRectMake(self.bgView.frame.origin.x, view.bounds.height-200, self.bgView.bounds.size.width, self.bgView.bounds.size.height)
        }
    }
    ...
    private func  initUserInterface()->Void{
        ...
        bgView.frame=CGRectMake(0, height, width, 200)
        ...
        cancle.addTarget(self, action: #selector(WBAdressPicker.cancle), forControlEvents: .TouchUpInside)
        ...
        done.addTarget(self, action: #selector(WBAdressPicker.done), forControlEvents: .TouchUpInside)
        bgView.addSubview(done)
        
        // 地址选择器
        picker.frame=CGRectMake(0, 40, width, 160)
